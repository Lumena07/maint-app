...
type EditModalProps = {
  isOpen: boolean;
  onClose: () => void;
  item: MaintenanceTask | Component | null;
  type: "Task" | "Component";
  onSave: (item: MaintenanceTask | Component) => void;
};

export const EditModal = ({ isOpen, onClose, item, type, onSave }: EditModalProps) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-bold mb-4">
          Edit {type}: {type === "Task" ? (item as MaintenanceTask).title : (item as Component).name}
        </h2>
